...
if (!$loggedInId) {
	$controller->command("renderPartial", "loginFullPagePrivate.htm"); 	
} else { 
  $thisPage = $controller->getVar("page"); 
  if (!$thisPage) $thisPage = "private_nominate";
  $tpl_title = $controller->command("getPageTitle", $thisPage); 
  $tpl_class = $controller->command("getPageCssClass", $thisPage);
?>
...
<html lang="en">
  ...
  <body>
    ...
    <?php  
       $loggedInId = $controller->command("getUserIdFromUserLoggedInTable");	
       $thisPage = $controller->getVar("page"); 
       if (!$thisPage) $thisPage = "private_reservations";
       if ($loggedInId && $thisPage == 'login') $thisPage = "private_reservations";
