...
function getPathToWhereThumbnailImageShouldGoServerPath($linkToExistingImage=false) {
...
// str_replace and therefore necessitates the function getPathToWhereThumbnailImageShouldGoServerPath.php
//
//			$pathToWhereThumbnailShouldGo = str_replace(".", "_thumb.", $pathToWhereThumbnailShouldGo);
			...
			$controller->error("In getPathToWhereThumbnailImageShouldGoServerPath we were given the path '$linkToExistingImage' which should be the path to an existing file, but no such file exists."); 
		}
	} else {
		$controller->error("In getPathToWhereThumbnailImageShouldGoServerPath we expected to be given a valid path to an existing image, but we were given nothing."); 
	}
}
