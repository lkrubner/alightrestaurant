...
function getPathToWhereThumbnailImageBiggerShouldGo($linkToExistingImage=false) {
...
// str_replace and therefore necessitates the function getPathToWhereThumbnailImageBiggerShouldGo.php
//
//			$pathToWhereThumbnailShouldGo = str_replace(".", "_thumb.", $pathToWhereThumbnailShouldGo);
			...
			$controller->error("In getPathToWhereThumbnailImageBiggerShouldGo we were given the path '$linkToExistingImage' which should be the path to an existing file, but no such file exists."); 
		}
	} else {
		$controller->error("In getPathToWhereThumbnailImageBiggerShouldGo we expected to be given a valid path to an existing image, but we were given nothing."); 
	}
}
