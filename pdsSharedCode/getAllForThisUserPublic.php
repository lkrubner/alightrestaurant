...
function getAllForThisUserPublic($databaseTable=false, $subForm=false, $orderBy="id", $limit=100) {
	...
	//
	//		< ?php getAllForThisUserPublic("dentists", "list_dentists.htm", "last_name"); ? >
	//
	...
	//
	// 		< ?php getAllForThisUserPublic("dentists", "list_dentists.htm", "last_name desc"); ? >
	
	global $controller; 
	
	if ($databaseTable) {
		if ($subForm) {			
			$id_users = $controller->getVar("id"); 
			...
			if ($id_users) {			
				...
				if ($result) {
					if (mysql_num_rows($result) > 0) {
						$controller->command("loopPublic", $result, "importForm", $subForm);
					} else {
			...
			$controller->error("In getAllForThisUserPublic, we were not given the name of a sub form to use to format each item."); 
		}
	} else {
		$controller->error("In getAllForThisUserPublic, we were not told which database table we should get info from."); 
	}
}
