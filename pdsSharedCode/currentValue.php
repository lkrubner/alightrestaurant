...
function currentValue($key=false, $format=false, $return=false, $noError=false) {
	...
	if (is_string($key)) {	
		...
		// 06-08-07 - the unfiltered nature of this next line is cause me all kinds of
		// problems. For instance, results have not been hit with stripslases(). Also,
		// image uploads record the PHP temporary file name for a file. Would anything
		// terrible happen if I commented this out? 
//		if (!$entry) $entry = $_SESSION["formInputs"]; 
		...
		if (!$noError) $controller->error("In currentValue we needed to be told a key to get from the current array, but we got nothing."); 	
	}
}
