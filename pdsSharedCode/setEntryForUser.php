...
function setEntryForUser($databaseTable="users", $alternateFieldToMatch=false, $id=false) {
	// 05-15-08 - adapted from setEntryForUser - this function assumes that 
	// the users id is the id to use, unless overridden by the id that can
	// be set as a function parameter. 
	// 
	// 
	// 
	...
	$singletonFormValuesObject = & $controller->getObject("SingletonFormValues", "setEntryForUser"); 
	$pageInfoObject = & $controller->getObject("SingletonPageInfo", "setEntryForUser"); 
	...
	if (!$databaseTable) {
		$controller->error("In setEntryForUser we expected to be told what database table to look for, but we were not."); 
		return false; 
	}
...
//
// 05-15-08 - adding this line for this new function
	if (!$id) $id = $controller->command("getUserIdFromUserLoggedInTable"); 
	...
	if (!$userId) {
		// 11-06-06 - this is not always an error, there may be times setEntryForUser is 
