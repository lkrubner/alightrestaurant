...
function getArrayOfForbiddenValuesFromForbiddenValuesFile() {
	// 05-14-07 - this is a copy of getArrayOfRequiredValuesFromForbiddenValuesFile
	// It is an exact copy. The code should probably be refactored so that we only
	// need one function, the file name could become a function argument, as it is
	...
	$pathToRequiredValuesFile = $pathToSiteSpecificFiles."forbiddenValuesForFields.php";
	...
	if (@ file_exists($pathToRequiredValuesFile)) {
		...
		if (is_array($arrayOfLinesFromFile)) {
			...
			$controller->error("In getArrayOfForbiddenValuesFromForbiddenValuesFile() we found the file requiredValuesForFields.php and yet for some reason we were not able to read an array of lines from it using file()."); 
		}					
	} else {
		return "no file"; 	
	}
