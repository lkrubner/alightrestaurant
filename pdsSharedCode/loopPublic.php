...
function loopPublic($resourcePointerToDatabaseResult=false, $nameOfActionToBeUsedToProcessEachRow=false, $param1=false, $keyForStorageInSingleton=false) {
	...
	if (is_resource($resourcePointerToDatabaseResult)) {	
		...
		for ($i=0; $i < $howManyItemsToLoopOver; $i++) {				
			$row = $controller->command("getRowPublic", $resourcePointerToDatabaseResult); 
