...
function setDataForPageForUser($databaseTable="users", $alternateFieldToMatch=false) {
	// 06-06-08 - this function is identical to setEntryForPageForUser in every way, 
	// except that it only looks for editId, it does not also look for "id". 
	//
	// I've realized the important of having a variable that is standard throughout the code that 
	// always refers to content and never, ever, refers to the user. Thus I've been 
	// introducing "editId" as a variable that can be passed through URLs 
	// to refer to content. "id" should be left only to referring to users. The old 
	// functions I wrote, like setEntryForPageForUser, are causing bugs because they look for 
	// both "id" and "editId". So now I'm creating functions that respond only to one of those variables.
	// 
	// 
	...
	$singletonFormValuesObject = & $controller->getObject("SingletonFormValues", "setDataForPageForUser"); 
	$pageInfoObject = & $controller->getObject("SingletonPageInfo", "setDataForPageForUser"); 
	...
	if (!$databaseTable) {
		$controller->error("In setDataForPageForUser we expected to be told what database table to look for, but we were not."); 
		return false; 
	}
	...
	if (!$userId) {
		// 11-06-06 - this is not always an error, there may be times setDataForPageForUser is 
