...
function userLogin($formInputs=false) {	
  ...
  if (!$loggedInId) {
    ...
    if (mysqli_num_rows($result) > 0) {
      $controller->addToResults("We are logging you in now."); 
    ...
    // 2014-01-25 - I find it counter-intuitive that a user is considered active if the
    // active field is set to 'null'. However, that is a decision that Joe made and I think
    // we would introduce some bugs if we tried to change it. 
    if ($row["active"] != false) {
      $controller->addToResults("Sadly, it appears that your account was never activated. Please contact us directly."); 
    }

    $realUsername = $row["email"];
    $realPassword = $row["password"];
    if(!$realPassword) $realPassword = $row["pass"];
